<div align="center">

<!-- Typing SVG Banner -->
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=28&pause=1000&color=6EE7B7&center=true&vCenter=true&width=600&lines=Hi+there%2C+I'm+Huang+Husky+%F0%9F%8C%9F;Welcome+to+my+digital+garden+%F0%9F%8C%BF;Where+code+meets+curiosity+%E2%9C%A8;Always+learning%2C+always+building+%F0%9F%9A%80" alt="Typing SVG" />

<br/>

<!-- Animated Wave -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=120&section=header&animation=twinkling" />

</div>

---

<!-- Profile Views + Social -->
<div align="center">

[![Profile Views](https://komarev.com/ghpvc/?username=huang-husky&color=6EE7B7&style=flat-square&label=Visitors)](https://sage-lume.vercel.app)
[![Blog](https://img.shields.io/badge/📖_Blog-sage--lume.vercel.app-6EE7B7?style=flat-square)](https://sage-lume.vercel.app)
[![GitHub](https://img.shields.io/badge/GitHub-huang--husky-181717?style=flat-square&logo=github)](https://github.com/huang-husky)

</div>

---

## 🌿 About This Space

> *"A blog is a garden — you plant seeds of thought, water them with curiosity, and let ideas grow wild."*

This is my personal digital garden — a place where I explore **Computer Science**, **Finance**, **Mathematics**, and the occasional rabbit hole that ties them all together.

I write because writing forces clarity. I publish because clarity deserves an audience.

---

## 🗂️ What You'll Find Here

```
sage-lume.vercel.app/
├── 💻  Tech & CS        → Algorithms, systems, dev tools
├── 📈  Finance & Quant  → Markets, models, data
├── 🔢  Math Corner      → Proofs, puzzles, intuitions
├── 📓  Reading Notes    → Papers & books I've digested
└── 🌱  Life & Learning  → Reflections on the journey
```

---

## 📡 Latest Posts

<!-- BLOG-POST-LIST:START -->
> 🔄 *Auto-updating via GitHub Actions — newest posts appear here automatically.*

<!-- You can use: https://github.com/gautamkrishnar/blog-post-workflow -->
<!-- BLOG-POST-LIST:END -->

➡️ **[Read all posts →](https://sage-lume.vercel.app)**

---

## 🛠️ Built With

<div align="center">

| Layer | Tech |
|:---:|:---:|
| Framework | ![Next.js](https://img.shields.io/badge/Next.js-000?style=flat-square&logo=next.js) |
| Hosting | ![Vercel](https://img.shields.io/badge/Vercel-000?style=flat-square&logo=vercel) |
| Styling | ![Tailwind](https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white) |
| Content | ![Markdown](https://img.shields.io/badge/MDX-1B1F24?style=flat-square&logo=mdx) |

</div>

---

## 📊 GitHub Activity

<div align="center">

<img height="160" src="https://github-readme-stats.vercel.app/api?username=huang-husky&show_icons=true&theme=nord&hide_border=true&count_private=true" />
<img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=huang-husky&layout=compact&theme=nord&hide_border=true" />

</div>

<div align="center">

<img width="80%" src="https://github-readme-activity-graph.vercel.app/graph?username=huang-husky&theme=nord&hide_border=true&area=true" />

</div>

---

## 💌 Get In Touch

If a post made you think differently, I'd genuinely love to hear from you.

<div align="center">

[![Visit Blog](https://img.shields.io/badge/🌐_Visit_Blog-sage--lume.vercel.app-6EE7B7?style=for-the-badge)](https://sage-lume.vercel.app)

</div>

---

<div align="center">

<!-- Footer wave -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=100&section=footer" />

<sub>Made with 🍵 and late-night curiosity · Updated automatically</sub>

</div>
